...
<div class="container mt-3">
    ...
    @endif
    <form method="post" action="{{ route('admin.video.store') }}" enctype="multipart/form-data" autocomplete="off">
        @csrf
        <p class="text-center">* fields are compulsory</p>
        <div class="form-group py-3">
            <label for="topic" class="form-label">Chapter Name *</label>
            <input type="text" class="form-control" name="topic" id="topic" value="{{ old('topic') }}" required>
        </div>
        <div class="form-group py-3">
        ...
        <div class="form-group py-3">
            ...
            <select name="demo" id="demo" class="form-select" data-live-search="true" required>
                ...
                @foreach($demoArray as $key => $value)
                    <option value="{{ $value }}" {{ old('demo') == $value ? 'selected' : '' }}>{{ $key }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group py-3">
