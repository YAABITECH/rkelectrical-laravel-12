...
@section('content')
    <div class="container mt-3">
        <h1 class="text-center text-primary fs-5 lh-lg">Practice Subtopic List</h1>
        ...
        <a class="btn btn-primary btn-sm" href="{{ route('admin.practice.subtopic.create') }}" role="button"><i class="fa-solid fa-plus"></i></a>
        ...
        <ul class="list-group" id="sortable">
            @foreach ($subtopic as $subtopic)
                    <li data_id="{{ $subtopic->id }}" class="chorder list-group-item">{{ $subtopic->name }}</li>
            @endforeach
        </ul>
    </div>
...
<script>
    var sortableList = Sortable.create(document.getElementById("sortable"), {
        onEnd: function (evt) {
            ...
            $.ajax({
                url: "/admin/practice/subtopic/updatePriority",
                type: "POST",
                data: formData,
                processData: false,
                contentType: false,
