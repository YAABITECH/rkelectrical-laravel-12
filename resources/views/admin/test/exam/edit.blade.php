...
@section('xmt_tit', 'Edit Test Exam | Admin')
...
<div class="container mt-3">
    <h1 class="text-center text-primary fs-5 lh-lg">Edit Test Exam</h1>
    ...
    <form method="post" action="{{ route('admin.test.exam.update', $exam->id) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="form-group py-3">
            ...
            <input type="text" class="form-control" value="{{ $exam->getSeries->name }}" readonly>
            <input type="hidden" name="series_id" id="series_id" value="{{ $exam->getSeries->id }}">
        </div>
        <div class="form-group py-3">
            <label for="name" class="form-label">Name *</label>
            <input type="text" class="form-control" name="name" id="name" value="{{ old('name',$exam->name) }}" required>
        </div>
        <div class="form-group py-3">
            <label for="tagline" class="form-label">Tagline</label>
            <input type="text" class="form-control" name="tagline" id="tagline" value="{{ old('tagline', $exam->tagline) }}">
        </div>
        <div class="form-group py-3">
            <label for="description" class="form-label">Description</label>
            <textarea name="description" class="yaTinyEditor" id="description" rows="5">{{ old('description', $exam->description) }}</textarea>
        </div>
        <div class="form-group py-3">
            <label for="instruction" class="form-label">General Instructions</label>
            <textarea name="instruction" class="yaTinyEditor" id="instruction" rows="5">{{ old('instruction', $exam->instruction) }}</textarea>
        </div>
        <div class="row">
            <div class="col-md-3 py-3">
                <label for="total_questions" class="form-label">Total Questions</label>
                <input type="number" class="form-control" name="total_questions" id="total_questions" value="{{ old('total_questions', $exam->total_questions) }}">
            </div>
            <div class="col-md-3 py-3">
                <label for="marks" class="form-label">Marks</label>
                <input type="number" class="form-control" name="marks" id="marks" value="{{ old('marks', $exam->marks) }}">
            </div>
            <div class="col-md-3 py-3">
                <label for="duration" class="form-label">Duration (mins)</label>
                <input type="number" class="form-control" name="duration" id="duration" value="{{ old('duration', $exam->duration) }}">
            </div>
            <div class="col-md-3 py-3">
                <label for="attempt_limit" class="form-label">Attempt Limit</label>
                <input type="number" class="form-control" name="attempt_limit" id="attempt_limit" value="{{ old('attempt_limit', $exam->attempt_limit) }}">
            </div>
        </div>
        <div class="form-group py-3">
            <div class="form-check">
                <input class="form-check-input" type="checkbox" value="1" id="has_negative_marks" name="has_negative_marks" {{ old('has_negative_marks', $exam->has_negative_marks) ? 'checked' : '' }}>
        ...
        <div class="form-group py-3">
            ...
            <select class="form-select" name="difficulty" id="difficulty" required>
                @foreach(['easy','medium','hard','mixed'] as $value)
                    <option value="{{ $value }}"
                        {{ old('difficulty', $exam->difficulty) == $value ? 'selected' : '' }}>
                        {{ ucfirst($value) }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group py-3">
            ...
            <select class="form-select" name="status" id="status">
                @foreach(['draft','upcoming','active','archive'] as $value)
                    <option value="{{ $value }}"
                        {{ old('status', $exam->status) == $value ? 'selected' : '' }}>
                        {{ ucfirst($value) }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group py-3 d-none" id="start_at_div">
            ...
            <div class="input-group date datetimepicker-input" id="startpicker" data-target-input="nearest">
                <input type="text" class="form-control datetimepicker-input"
                    ...
                    value="{{ old('start_at', $exam->start_at ? $exam->start_at->format('d-m-Y g:i A') : '') }}">
        ...
        <div class="form-group py-3">
            ...
            <div class="input-group date datetimepicker-input" id="endpicker" data-target-input="nearest">
                <input type="text" class="form-control datetimepicker-input"
                    ...
                    value="{{ old('end_at', $exam->end_at ? $exam->end_at->format('d-m-Y g:i A') : '') }}">
        ...
        <div class="form-group py-3">
            <div class="form-check">
                <input class="form-check-input" type="checkbox" value="1" id="is_demo" name="is_demo" {{ old('is_demo', $exam->is_demo) ? 'checked' : '' }}>
                <label class="form-check-label" for="is_demo">Is Demo?</label>
            </div>
        </div>
        <div class="form-group py-3">
            <button type="submit" class="btn btn-block btn-primary">Update Test Exam</button>
        </div>
    </form>
</div>
@endsection
