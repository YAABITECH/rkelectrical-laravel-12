...
@section('content')
    <div class="container mt-3">
        <h1 class="text-center text-primary fs-5 lh-lg">Practice Topic List</h1>
        ...
        <a class="btn btn-primary btn-sm" href="{{ route('admin.practice.topic.create') }}" role="button"><i class="fa-solid fa-plus"></i></a>
        ...
        <ul class="list-group" id="sortable">
            @foreach ($topic as $topic)
                    <li data_id="{{ $topic->id }}" class="chorder list-group-item">{{ $topic->name }}</li>
            @endforeach
        </ul>
    </div>
...
<script>
    var sortableList = Sortable.create(document.getElementById("sortable"), {
        onEnd: function (evt) {
            ...
            $.ajax({
                url: "/admin/practice/topic/updatePriority",
                type: "POST",
                data: formData,
                processData: false,
                contentType: false,
